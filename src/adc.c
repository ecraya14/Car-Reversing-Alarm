...
/* ================================
   This file provides the following functions
      ...
      MeasureVoltageDiff - compare two single-end voltages
			ADC_Cal   - calibration the ADC
			
	 The variables sres, measuredVoltageLEDOn, measuredVoltageLEDOff and difference 
	 are also declared and
...
volatile uint16_t measuredVoltageLEDOn;
volatile uint16_t measuredVoltageLEDOff;
volatile double difference ; 
...
void MeasureVoltage(void) {
	 ...
	 // Write to ADC0_SC1A 
	 //   0 --> AIEN Conversion interrupt disabled
...
void MeasureVoltageDiff(void) {
	...
	//delay
	waitSysTickCounter(10) ;  // cycle every 1 ms or more? test
	MeasureVoltage();
	measuredVoltageLEDOn = sres;
	//turn IR LED Off
	PTB-> PSOR = MASK(IR_POS) ;
	//delay
	waitSysTickCounter(10) ;  // cycle every 1 ms or more? test
	MeasureVoltage();
	measuredVoltageLEDOff = sres;
	
	//if detecting object then ON is less than OFF
	difference = (double) measuredVoltageLEDOn / (double) measuredVoltageLEDOff;
